...
"""
        ...
        # 3. 사용자 생성 확인
        cursor.execute("SELECT id, email FROM User WHERE id = 'admin_legacy_user'")
        user_result = cursor.fetchone()
        if user_result:
            print(f"사용자 확인: {user_result[0]} ({user_result[1]})")
        else:
            print("사용자 생성 실패!")
            return
