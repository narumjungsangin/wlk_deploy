...
def main():
    ...
    try:
        ...
        for board_slug, board_data in all_data.items():
            ...
            print(f"\n[{board_data['board_name']}] 강제 삽입 시작...")
            
            # 기존 데이터 삭제
            category_name = BOARD_MAPPING[board_slug]["category"]
            cursor.execute(f"DELETE FROM Post WHERE category = '{category_name}'")
            conn.commit()
