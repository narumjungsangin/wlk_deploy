...
def main():
    ...
    try:
        ...
        posts_data = [
            # 정보나눔터 (info)
            ('legacy_info_1', 'info', 'general', '피아노 조율', 'Mike Bratcher 317-371-5747 웨라에 사는 분은 아니어서 이 분이 웨라에 오실 때 맞춰서 시간 잡고 있어요.'),
            ('legacy_info_2', 'info', 'general', 'Pest Control', '웨스트라피엣 한인커뮤니티 단톡방 질문과 답변 공유합니다.'),
            ('legacy_info_3', 'info', 'general', '치과 추천해주세요', '정기점검으로 일반 성인 치과 추천 부탁해요'),
            
            # 구인구직 (jobs)
            ('legacy_jobs_1', 'jobs', None, '케어(아이)', '아이 돌봄 경력 있는 분 찾습니다.'),
            ('legacy_jobs_2', 'jobs', None, 'Admin Officer, 웨스트라피엣 학교', '웨스트라피엣 학교에서 Admin Officer를 채용합니다.'),
            ('legacy_jobs_3', 'jobs', None, '번역(영어)', '영어 번역 가능한 분 찾습니다.'),
            
            # 과외 (tutoring)
            ('legacy_tutoring_1', 'tutoring', None, '해외 유학', '해외 유학 상담 가능합니다.'),
            ('legacy_tutoring_2', 'tutoring', None, '튜터링 (수학과학)', '수학과 과학 튜터링 가능합니다.'),
            ('legacy_tutoring_3', 'tutoring', None, '피아노 레슨', '피아노 레슨 가능합니다.'),
            ('legacy_tutoring_4', 'tutoring', None, '대학생 과외', '대학생 과외 가능합니다.'),
        ...
        print("Creating posts with slug-based categories...")
        ...
        print(f"\nSlug-based migration completed!")
        print(f"Admin login: admin@wlafayettekorea.org / admin123")
        print(f"Now all categories should be visible on the website!")
