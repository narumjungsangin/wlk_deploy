...
def main():
    ...
    try:
        ...
        # 기존 FAQ 데이터 모두 삭제
        print("기존 FAQ 데이터 모두 삭제...")
        cursor.execute("DELETE FROM Post WHERE category = 'FAQ'")
        conn.commit()
...
"""
        ...
        # 최신 게시물 확인
        cursor.execute("SELECT title FROM Post WHERE category = 'FAQ' ORDER BY createdAt DESC LIMIT 3")
