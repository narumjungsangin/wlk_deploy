...
def main():
    ...
    try:
        ...
        print(f"정보나눔터 강제 삽입 시작...")
        
        # 기존 정보나눔터 데이터 삭제
        cursor.execute("DELETE FROM Post WHERE category = '정보나눔터'")
        conn.commit()
...
"""
        ...
        print(f"\n정보나눔터: {success_count}개 성공")
        
        # 결과 확인
        cursor.execute("SELECT COUNT(*) FROM Post WHERE category = '정보나눔터'")
        info_count = cursor.fetchone()[0]
        print(f"현재 정보나눔터 게시물: {info_count}개")
        
        # 게시물 목록 확인
        cursor.execute("SELECT title FROM Post WHERE category = '정보나눔터' ORDER BY createdAt DESC")
        info_posts = cursor.fetchall()
        print("\n정보나눔터 게시물:")
