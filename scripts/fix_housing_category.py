...
def main():
    ...
    try:
        ...
        if board_slug not in all_data:
            print("housing 데이터가 없습니다.")
            return
            
        board_data = all_data[board_slug]
        print(f"Housing 강제 삽입 시작...")
        
        # 기존 Housing 데이터 삭제
        cursor.execute("DELETE FROM Post WHERE category = 'Housing'")
        conn.commit()
...
"""
        ...
        print(f"\nHousing: {success_count}개 성공")
        
        # 결과 확인
        cursor.execute("SELECT COUNT(*) FROM Post WHERE category = 'Housing'")
        housing_count = cursor.fetchone()[0]
        print(f"현재 Housing 게시물: {housing_count}개")
        
        # 게시물 목록 확인
        cursor.execute("SELECT title FROM Post WHERE category = 'Housing' ORDER BY createdAt DESC")
        housing_posts = cursor.fetchall()
        print("\nHousing 게시물:")
