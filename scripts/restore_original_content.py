...
def main():
    print("=== RESTORE ORIGINAL CONTENT ===\n")
    ...
    if not all_boards_file.exists():
        print(f"Error: Crawled data file not found.")
        return
    ...
    try:
        ...
        for board_slug, board_data in all_data.items():
            ...
            category_slug = BOARD_MAPPING[board_slug]
            ...
            for post in posts:
                try:
                    ...
                    cursor.execute("""
                        ...
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, NOW())
                    """, (post_id, category_slug, subcategory, title, content, admin_id, 0, created_at))
                    conn.commit()
                    success_count += 1
                    ...
                    for i, comment in enumerate(comments):
                        if comment.get('content', '').strip():
                            ...
                            cursor.execute("""
                                ...
                                VALUES (%s, %s, %s, %s, NOW())
                            """, (comment_id, comment['content'], post_id, admin_id))
                            conn.commit()
                    
                    print(f"  성공: {post.get('title', '')[:30]}...")
        ...
        for i, post in enumerate(faq_posts, 1):
            ...
            """, (post_id, 'faq', None, post['title'], post['content'], admin_id, 0))
            conn.commit()
        
        print(f"FAQ: {len(faq_posts)}개 성공")
